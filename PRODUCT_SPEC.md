...
#### 第一层：LLM 语义理解（System Prompt 内置）

在象长老的 System Prompt 中，明确要求 Claude 识别以下危险信号并切换到安全回应模式：
- 自伤/自杀意图（"不想活了"、"想伤害自己"、"活着好累"、"要是我不在了妈妈会不会开心"）
- 家暴/虐待描述（"爸爸打我"、"妈妈把我锁起来"、"不让我吃饭"）
- 严重霸凌（被打、被勒索、被拍照威胁）
- 性侵相关

LLM 能理解隐晦表达（如"活着好累"），这是关键词匹配做不到的。

#### 第二层：关键词前置过滤（代码硬性拦截）

在孩子的消息发送给 LLM **之前**，先做一次关键词扫描作为确定性兜底。即使 LLM 有极小概率忽略 System Prompt 指令，关键词命中就一定会触发安全回应。

关键词列表参考（`lib/safety.ts`）：
- 自伤类：不想活、想死、自杀、割腕、跳楼、吃药死
- 家暴类：打我、用棍子、用皮带、烫我、锁起来、不让吃饭
- 性侵类：摸我、脱衣服、不能告诉别人
- 霸凌类：勒索、拍裸照

#### 第三层：安全回应 + 分类通知（核心设计）

检测到危险信号后，需要**根据施害者是谁**走不同路径：

**路径 A：危险来源是家庭外部（自伤、校园霸凌等）**
```
→ 象长老切换到安全回应模式
→ 显示求助热线卡片
→ ✅ 自动邮件通知家长（Phase 3 实现）
...
**如何判断走哪条路径？** LLM 在对话中可以理解语境——孩子提到"爸爸打我"、"妈妈不让我出门"时，施害者是家长；孩子说"同学打我"、"我不想活了"时，施害者不是家长。这个判断交给 Claude 在对话中完成。

#### 安全回应模式的具体行为

**对孩子说的话**（温和但明确）：
> "象长老听到了你说的话。你愿意告诉象长老这些，很勇敢。
> 这件事很重要，不是你的错。
> 但象长老是一个 App，没有办法真正保护你。
> 有人可以——请你告诉一个你信任的大人，比如老师、亲戚、邻居。
> 你也可以打这个电话，会有真正的大人帮你：
> 12355（全国青少年服务热线）
> 你不是一个人。"
...
- ❌ 不继续追问细节
- ❌ 不试图用 AI 解决问题
- ❌ 不弹"你的话包含不当内容"之类的错误提示（会吓到孩子、让孩子不敢再说）
- ❌ 不生成话卡（这种情况不适合通过话卡传达）

#### 家长通知邮件的设计（Phase 3 实现）

当走路径 A（危险来源非家长）时，给家长发送**模糊提醒**而非孩子原话：

> **来自象长老的提醒**
>
> 你的孩子今天跟象长老聊了一会儿。
> 象长老注意到孩子可能正在经历一些让他/她不太好受的事情。
>
> 这不是紧急警报，但象长老建议你找一个安静的、不会被打断的时间，
> 温和地问问孩子最近的感受。
>
> 如果你需要专业支持，可以拨打：
> 12355（全国青少年服务热线）
...
**邮件不包含孩子的原话**——这是为了保护孩子对产品的信任。如果孩子知道"说的每句话都会被转发给妈妈"，就不会再真实表达了。邮件只起到"提醒家长关注"的作用。

#### 各阶段实现范围

| 安全功能 | 实现阶段 |
|---------|---------|
| System Prompt 中的安全指令 | Phase 1a |
| 关键词前置过滤（`lib/safety.ts`） | Phase 1a |
| 安全回应文本 + 求助热线卡片 | Phase 1a |
| 区分路径 A / 路径 B | Phase 1a（LLM 判断） |
| 自动邮件通知家长（路径 A） | Phase 3（需要用户体系 + Resend） |
